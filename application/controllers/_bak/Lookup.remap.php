...
class Lookup extends CI_Controller {
	...
	{
		$this->aData['sLookupCode'] = $method;
		$nCountParams = count($params);
		if ($nCountParams > 0) 
		{
			$func = $params[0];
			if ($func == 'form')
			{
				$sState = $this->lang->line('form');
			}
			else
			{
				$func = 'list';
				$sState = $this->lang->line('list');
			}
		}
		else 
		{
		...
		if ($nCountParams > 1) $nId = (int) $params[1];
		...
		$this->aData['contentheader01'] = $this->lang->line('menu_lookup');
		$this->aData['contentheader02'] = $sState;
		...
		case 'form':
			...
			else
			{
				...
				$this->aBreadcrumb['link'][1] = $this->config->item('uri_lookup') . $this->aData['aLkp']['lkp'];
