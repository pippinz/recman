	...
	<div class="box box-black-topborder">
		<div class="box-header">
			...
			<span id="divdocadd" style="float:right;<?php echo ($mode == 'insert') ? 'display:none;' : '';?>">
				<button id="btnadd" name="btnadd" type="button" class="btn btn-primary btn-primary-small" onclick="<?php echo 'location.href=\'' . $this->config->item('uri_file_doc_form') . $id . '/\'';?>">Add New Document</button>
			<span>
		</div>
		<div class="box-body">
		  <table id="example1" class="table table-bordered table-striped">
			...
			foreach ($record as $r) { 
				echo '<tr>' .
					...
					'<td>'. $r['pic'] .'</td>' .
					'<td>'. '<a href="'. $this->config->item('uri_file_doc_form') . $r['fileid'] .'/' . $r['id'] . '/">' . $this->config->item('ico_edit') .'</a></td>' .
					'</tr>' ;
			}
			?>
			</tbody>
